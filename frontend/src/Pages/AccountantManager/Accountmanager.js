        ...
        <>
            ...
            <div className={styles.outer}>
                {gsnList.map((item, index) => {
                    ...
                    const isApprovedByAllFour = !!item.GeneralManagerSigned && !!item.PurchaseManagerSigned && !!item.StoreManagerSigned && !!item.AccountManagerSigned;
                    const statusText = isApprovedByAllFour ? "(Approved)" : "(Pending Approval)";

                    const isAccountManagerEnabled = !!item.GeneralManagerSigned && !!item.PurchaseManagerSigned && !!item.StoreManagerSigned && !!grinNo;
                    ...
                    return (
                        <div key={_id} id={`item-div-${_id}`} className={styles.show}>
                            ...
                            >
                                {partyName || 'N/A'}
                                <span style={{ marginLeft: '10px', fontSize: '0.8em', color: isApprovedByAllFour ? 'green' : 'orange' }}>
                                    {statusText}
                                </span>
                            </h2>
